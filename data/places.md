...
| `house_kitchen_chest` | (-267, 67, 569) | **Deposit wheat here.** Upper chest right of bed. Approach via pathfind to (-267, 65, 570) range=1 — reach works from y=65 up to the y=67 chest. |
...
1. **Suppress `look_at`** (the nearest-player tracker) for the duration — a background yaw change during step 4 sends the bot east into the furnace.
2. Pathfind to `house_center` (-268, 65, 572), range=0
3. **Verify on the orientation block.** y must be ≈ 65.0 and xz within 1.5 of (-268, 572). If y≥65.5, pathfinder put the bot on a chest — re-pathfind via bedside (-268, 65, 570) range=1, then to center. If xz is off, abort.
4. `look yaw=1.5708 pitch=0` (west) AND wait for `rawState.yaw` to converge within 0.25 rad. **Abort if it doesn't** — do not proceed on hope.
5. `walk_until axis=x target=-275 direction=lte max_ms=8000`, bail on HP drop or death
6. Verify: x ≈ -275, y=64, deaths unchanged. Should be standing on `outside_orientation`.
...
1. **Suppress `look_at`** for the duration (same reason as exit).
2. Pathfind to `outside_orientation` (-275, 64, 572), range=0
3. **Verify on the orientation block** (xz within 1.5 of (-275, 572), y ≈ 64). Abort if off-pad — activating the door from the wrong angle misses and the forward-push walks past.
4. `look yaw=-1.5708 pitch=0` (east) AND wait for `rawState.yaw` to converge. Abort on failure.
5. `activate_block` on `house_door` (-272, 65, 572); pause ~300ms for the open packet
6. `walk_until axis=x target=-268 direction=gte max_ms=8000` (momentum lands at ≈ -267.3), bail on HP drop or death
7. Verify: x ≈ -268, z ≈ 572.5, deaths unchanged. Should be standing on `house_center`.
