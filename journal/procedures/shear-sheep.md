...
The `bot-ctl` action `activate_entity` defaults to `mode=double`. **However, the production `runShearSheep()` routine currently uses single-packet activation (`bot.activateEntity`), not double.** This is a known low-yield discrepancy given the A/B table above (single is near-zero per activation). The current routine uses single-packet; double is available via the `activate_entity` ctl action (`mode: 'double'`) if higher yield is needed.
