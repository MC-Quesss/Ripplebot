...
Implementation: `tossTrash()` in bot.js scans for items in `TRASH_ITEMS` (includes `poisonous_potato`) and tosses each trash stack in place wherever the bot currently stands. Runs automatically during wheat and potato harvests. Also exposed as `toss_trash` ctl command (added 2026-06-05).
