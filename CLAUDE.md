...
5. **Farming & crafting routines** (~3000–5700) — harvest wheat/potatoes (right-click replant), shear sheep, bake bread/potatoes (raw window_click crafting for modded GUIs), deposit/stash routines, food-safety sustain loop.
   ...
   **Multi-bot fire-duty coordination** (`FIRE_*` constants near `runSustainFarm`): when several bots run "keep the fire going" at once, they coordinate over in-game chat — the only channel shared across machines. Persona-voiced lines carry parseable cores: a roll call on start ("who's on fire duty?"), field claims ("I'll take the north field"), and stand-downs. The first two keepers split north/south field, extras supervise from the field edge and auto-promote when a field frees up; claim conflicts tie-break alphabetically by nickname. The `FIRE_*_RE` regexes and persona slots (`sustainRollCall`, `sustainClaimNorth/South`, `sustainSupervise`, `sustainStop`) must stay in sync — every line in those slots has to match its corresponding regex or other bots can't parse the announcement.
